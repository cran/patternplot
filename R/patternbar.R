...
#'
#' @details \code{patternbar} function offers flexible ways in doing bar charts.
#'   
...
patternbar<-function(data,x, y, group=NULL, xlab='', ylab='',  label.size=3.5,vjust=-1,
          pattern.type, pattern.line.size=1,frame.size=1, 
          ...
          pixel=1, density=rep(7, ifelse(is.null(group), length(x), length(unique(group)))),
				  frame.color=rep('black', ifelse(is.null(group), length(x), length(unique(group)))),
				  legend.type='h', legend.h=6, legend.y.pos=0.5, legend.pixel=0.1){
  ...
  if(is.null(group)){
    ...
    for (i in 1:dim(gdata)[1]){
      boxmatrix[[i]]<-matrix(c(gdata[i,"xmin"], 0,
                             ...
                             nrow=5, 
                             ncol=2, byrow=T)
      suppressWarnings(pattern(type=pattern.type2[i], density=4*density2[i], color=pattern.color2[i], pattern.line.size=pattern.line.size, background.color=background.color2[i], pixel=pixel))
    ...
    if(legend.type=='v'){
      ...
      for (i in 1:length(pattern.type)){
        legendbox[[i]]<-matrix(c(legend.x[1], legend.y[i],
                               ...
                               nrow=5, 
                               ncol=2, byrow=T)
        suppressWarnings(pattern(type=pattern.type[i], density=density[i], color=pattern.color[i], pattern.line.size=0.7*pattern.line.size, background.color=background.color[i], pixel=legend.pixel))
