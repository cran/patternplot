...
#'
#' @details \code{patternpie} function offers flexible ways of doing pie charts. 
#'   
...
patternpie<-function(group,pct,label,label.size=4, label.color='black',label.distance=1.2,pattern.type, pattern.color=rep('black', length(group)),
                    pattern.line.size=rep(5, length(group)), background.color=rep('white', length(group)), frame.color='black',frame.size=1, density=rep(10, length(group)), pixel=20){
  ...
  picdf<-list()
    for (i in 1:length(group)){
      ...
      if (sub( "_.*", "", pattern.type[i])=='Unicode'){
        picdata[[i]]<-imagetodf1(readPNG(paste(location,'/','Unicode',".png", sep='')),  as.matrix(piedata[[i]]),left = min(piedata[[i]]$x), right = max(piedata[[i]]$x),bottom = min(piedata[[i]]$y),top = max(piedata[[i]]$y))
      }else{
        picdata[[i]]<-imagetodf1(readPNG(paste(location,'/',pattern.type[i],".png", sep='')), as.matrix(piedata[[i]]),left = min(piedata[[i]]$x), right = max(piedata[[i]]$x),bottom = min(piedata[[i]]$y),top = max(piedata[[i]]$y))
      } 
