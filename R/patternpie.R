...
patternpie<-function(group,pct,label,label.size=4, label.color='black',label.distance=1.2,pattern.type, pattern.color=rep('black', length(group)),
                    pattern.line.size=1, background.color=rep('white', length(group)), frame.color='black',frame.size=1, pixel){
  ...
  picdf<-list()
    for (i in 1:length(group)){
      ...
      suppressWarnings(pattern(type=pattern.type[i], density=10, color=pattern.color[i], pattern.line.size=pattern.line.size, background.color=background.color[i], pixel=pixel))
